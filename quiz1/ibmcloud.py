...
def updatesalary():
    ...
    name = request.form.get("name")
    salary_data = []
    query1 = "UPDATE PEOPLE SET SALARY = '" + str(salary) + "' WHERE NAME = '"+ str(name)+ "'"
    stmt1 = ibm_db.exec_immediate(conn, query1)
    print("update: ", stmt1)
    query2 = "SELECT * FROM PEOPLE"
    stmt2 = ibm_db.exec_immediate(conn, query2)
    ...
    return render_template('update.html', table=salary_data, title='UpdateSalary')
...
def delete():
    ...
    deleted_data = []
    query1 = "DELETE FROM PEOPLE WHERE NAME = '" + str(dname)+"'"
    stmt1 = ibm_db.exec_immediate(conn, query1)
    # result = ibm_db.fetch_both(stmt1)
    query2 = "SELECT * FROM PEOPLE"
    stmt2 = ibm_db.exec_immediate(conn, query2)
